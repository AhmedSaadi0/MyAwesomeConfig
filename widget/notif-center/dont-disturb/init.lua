...
function naughty.config.notify_callback(args)
	if not dont_disturb then
		if not args.app_name:match("Clementine") and not args.app_name:match("Strawberry") and not args.app_name:match("Amarok") and not args.app_name:match("الاذان") then
